...
AstcTexture astcCompress(const LinearImage& source, AstcConfig config) {
    ...
    // For now we do not support 3D textures.
    constexpr int xdim_3d = 1, ydim_3d = 1, zdim_3d = 1;
    ...
    expand_block_artifact_suppression(xdim_2d, ydim_2d, 1, &ewp);
    ...
    const int xblocks = (xsize + xdim_2d - 1) / xdim_2d;
    const int yblocks = (ysize + ydim_2d - 1) / ydim_2d;
    const int zblocks = (zsize + zdim_3d - 1) / zdim_3d;

    uint32_t size = xblocks * yblocks * zblocks * 16;
    uint8_t* buffer = new uint8_t[size];

    encode_astc_image(input_image, nullptr, xdim_2d, ydim_2d, zdim_3d, &ewp, decode_mode,
